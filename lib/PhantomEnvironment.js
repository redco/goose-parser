...
PhantomEnvironment.prototype = _.create(Environment.prototype, /**@lends PhantomEnvironment*/{
    ...
    _rotateProxy: function () {
        ...
        if (Array.isArray(proxy)) {
            ...
            var promise = (typeof this._options.proxyRotator === 'function') ?
                this._options.proxyRotator.bind(this, proxy, currentProxy) :
                vow.resolve(_.sample(proxy));

            return promise
                .then(function (foundProxy) {
                    if (foundProxy == undefined) {
                        throw new Error('No proxy found');
                    }
                    return self._applyProxy(foundProxy);
                });
        }
    ...
    _handlePhantomEvents: function () {
        return vow.all([
            ...
            this._on('resourceError', function (resourceError) {
                debug('Navigation error %s %s', resourceError.url, resourceError.errorString);
            }.bind(this)),
            this._on('resourceReceived', function (response) {
                if (response.stage === 'end') {
                    //todo: store to cache to the filesystem
