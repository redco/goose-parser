    ...
    _handlePhantomEvents: function () {
        return vow.all([
            ...
            this._on('resourceRequested', /* @covignore */ function (requestData, request, allowedUrls, blockedUrls) {
                ...
                var allowed = !hasAllowedUrls || allowedUrls.reduce(function (lastValue, urlPattern) {
                        return lastValue || url.match(urlPattern) !== null
                    }, false);

                var blocked = false;
                if (!hasAllowedUrls && hasBlockedUrls) {
                    blocked = blockedUrls.reduce(function (lastValue, urlPattern) {
                        return lastValue || url.match(urlPattern) !== null;
                    }, blocked);
                }
                if (!allowed || blocked) {
