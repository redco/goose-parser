...
PhantomEnvironment.prototype = _.create(Environment.prototype, /**@lends PhantomEnvironment*/{
    ...
    waitForPage(timeout) {
        ...
        return deferred.promise().then(this.inject, this);
    },
    ...
    waitForQuery(uri, timeout) {
        ...
        const timeoutId = setTimeout(() => {
            ...
            this.removeRequestingAction(requestingAction);
            deferred.reject(new Error('Waiting request timeout'));
        }, timeout);

        this.addRequestingAction(requestingAction);
        debug('Added request callback');

        return deferred.promise();
    },

    addRequestingAction(action) {
        this._requestingActions.push(action);
    },

    removeRequestingAction(action) {
        this._requestingActions = this._requestingActions.filter(a => a !== action);
    },
    ...
    _handlePhantomEvents() {
        ...
        page.set('onError', (msg, trace) => {
            // we add errbacks while wait in concurrent promises to optimize failing. But sometimes
            // scripts on the page are failed during wait, it causes false alarms
            // todo: cover this shit with tests and fix "properly"
            // as workaround you can block loading of broken scripts on a parsing site
