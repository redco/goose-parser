...
var url = require('url');
var RedParser = require('../parser/RedParser');
var PaginateableParser = require('../parser/PaginateableParser');
 ...
 * @private
 */
PhantomParser.prototype.createInstance = function () {
 ...
 * @private
 */
PhantomParser.prototype.tearDownInstance = function () {
 ...
 * @private
 */
PhantomParser.prototype.setupInstance = function () {
 ...
 * @private
 */
PhantomParser.prototype.setup = function () {
    var deferred = vow.defer();
    var self = this;
    this
        .setupInstance()
        .then(function () {
            ...
            self.phantomJS.createPage(function (page) {
                self.page = page;
 ...
 * @param {String} [signal]
 */
PhantomParser.prototype.handleCrash = function (code, signal) {
 ...
 * @private
 */
PhantomParser.prototype.untilOnPage = function (check, value, then) {
    var page = this.page;
    var condition = false;
 ...
 * @private
 */
PhantomParser.prototype.goto = function (url) {
    var deferred = vow.defer();
    debug('.goto() url: ' + url);
 ...
 * @private
 */
PhantomParser.prototype.viewport = function (width, height) {
    var deferred = vow.defer();
 ...
 * @private
 */
PhantomParser.prototype.inject = function () {
    debug('.inject()-ing parser libs');
    var deferred = vow.defer();
    var files = [
        'src/lib/parser/debug.js',
        'src/lib/parser/RedParser.js',
        'src/lib/parser/PaginateableParser.js'
    ];
    var self = this;
    files.forEach(function (file) {
        debug('injecting file %s', file);
        self.page.injectJs(file);
    });
    deferred.resolve();
 ...
 * @private
 */
PhantomParser.prototype.wait = function (/* args */) {
    var deferred = vow.defer();
    var args = arguments;
    var self = this;
    ...
    if (typeof args[0] === 'string') {
        ...
        this.untilOnPage(
            ...
            function (present) {
                if (!present) {
                    deferred.reject();
                    self.onTimeout('timeout elapsed before selector "' + selector + '" became present');
                }
                else {
                    deferred.resolve();
                }
    ...
    // wait for on-page fn==value
        ...
        this.untilOnPage(
            ...
            function (val) {
                if (val !== value) {
                    deferred.reject();
                    self.onTimeout('timeout elapsed before fn===' + value);
                }
                else {
                    deferred.resolve();
                }
 ...
 * @private
 */
PhantomParser.prototype.on = function (type, callback) {
    var deferred = vow.defer();
 ...
 * @private
 */
PhantomParser.prototype._parse = function (rule) {
    ...
    this.page.evaluate(
        function (rule, DEBUG) {
            window.DEBUG = DEBUG;
            window.__parser = new PaginateableParser();
 ...
 * @returns {Promise}
 */
PhantomParser.prototype.parse = function (url, rule, pagination, waitScope) {
    ...
    this
        ...
        .then(this.on.bind(this, 'timeout', function (msg) {
            debug(msg);
            self.tearDownInstance();
        }))
        .then(this.on.bind(this, 'exit', function (code, signal) {
            debug('Phantom exited with code ' + code + ' and signal ' + signal);
        }))
        .then(this.on.bind(this, 'error', function (msg, trace) {
            phantomError('%s, trace %o', msg, trace);
        }))
        .then(this.on.bind(this, 'consoleMessage', function (msg) {
        ...
        .then(this.viewport.bind(this, this.options.screen.width, this.options.screen.height))
        ...
        .then(function (parsed) {
            var inDeferred = vow.defer();
            if (pagination === undefined) {
                inDeferred.resolve(parsed);
            }
             ...
             * @returns {Promise}
             */
            function paginate(parsed, rule, pg) {
                return self
                    .paginate(pg)
                    .then(self._takeScreenshot.bind(self, host, 'paginated.' + pg.page))
                    .then(self.getPage.bind(self, parsed, rule, pg))
                    .then(function (result) {
                        debug('Paginated %o', result);
                        var notEmpty;
                        if ((notEmpty = result.parsed.length !== 0) || extraTries > 0) {
                            if (notEmpty) {
                                extraTries = 2;
                                parsed = parsed.concat(result.parsed);
                                pg.page = result.page + 1;
                            } else {
            ...
            paginate(parsed, rule, pagination);
 ...
 * @private
 */
PhantomParser.prototype.getPage = function (previousPage, rule, pagination) {
 ...
 * @private
 */
PhantomParser.prototype.paginate = function (pagination) {
...
PhantomParser.prototype._takeScreenshot = function (host, fileName) {
    debug('._takeScreenshot()');
    var deferred = vow.defer();

    var self = this;
    var screenShotFilePath = this.options.reportsDir + host + '/';
    var screenShotFileName = screenShotFilePath + fileName + '.png';
    vowNode
        ...
        .then(function (err) {
            if (!err) {
                self.page.clipRect = {
                    left: 0,
                    top: 0,
                    width: self.options.screen.width,
                    height: self.options.screen.height
                };
                self.page.render(screenShotFileName);
                deferred.resolve();
            }
        });
    return deferred.promise();
 ...
 * @private
 */
PhantomParser.prototype._getHostName = function (uri) {
