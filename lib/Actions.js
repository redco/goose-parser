...
class Actions {
    ...
    async performAction(action, parentSelector, prevResult) {
        ...
        if (action.waitForQuery) {
            ...
            await this.performAction(waitAction, parentSelector, prevResult);
        }

        let casesResult;
        if (action.cases && action.type !== 'cases') {
            casesResult = await this._createInstance({
                    type: 'cases',
                    cases: action.cases
                },
                selector,
     ...
     * @returns {Promise}
     */
    async wait(evalFunction, checkerFunction, args, timeout, interval) {
    ...
    async scroll(interval) {
        ...
        return this._env.evaluateJs(interval, /* @covignore */ (interval) => {
