...
function Parser(options) {
    ...
    if (options.pagination) {
        var paginationOptions = _.cloneDeep(options.pagination);
        ...
        return this._env.evaluateJs(selector, filter, /* @covignore */ function(selector, filter) {
            ...
            return nodes.map(function(node) {
                switch (filter.type) {
                    ...
                    case 'attr':
                        const valueType = filter.value
                            .constructor
                            .toString()
                            .replace(/^function ([A-z]+)(.+)/i, '$1')
                            .toLowerCase();
                        if (valueType === 'object' && Array.isArray(filter.value.or)) {
                            const res =  filter.value.or.map(function(value) {
                                return node.getAttribute(value);
                            }).filter(Boolean);
                            return res.pop();
                        }
                        console.log(filter.value.constructor.toString().replace('function String() {[native code]}'));
