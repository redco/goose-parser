...
Parser.prototype = {
    ...
    _parseScope: function (rule, offset, actionsResults) {
        ...
        const format = results => {
            if ([this.TYPES.SIMPLE, this.TYPES.GET, this.TYPES.VALUE].indexOf(ruleType) !== -1) {
                if (rule.type !== 'array') {
                    return results.join(rule.separator || ' ');
                }

                // if rule.type is array but result is not array type
    ...
    _parseRow: function (options) {
        ...
        return this
            ...
            .then(function (results) {
                if (this.clearDom && this._paginator.getType() === 'scroll') {
