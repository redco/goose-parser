...
class Parser {
    ...
    async _parseScope(rule, offset, actionsResults) {
        ...
        switch (ruleType) {
            ...
            case RULE_TYPE.GET:
                results = this._storage.get(rule.get);
                break;
        ...
        const updateResultsInStore = results => {
            if (rule.set) {
                this._storage.set(rule.set, results);
            }
            if (rule.add) {
                const current = this._storage.get(rule.add) || [];
                current.push(results);
                this._storage.set(rule.add, current);
            }
            if (rule.unset) {
                this._storage.unset(rule.unset);
            }
            return results;
        };
    ...
    async _parseGridRule(rule, offset) {
        ...
        let nodesCount = await this._env.evaluateJs(
            ...
            /* istanbul ignore next */ function(domSelector, jsSelector) {
    ...
    async _parseRow(options) {
        ...
        if (this.clearDom) {
            ...
            await this._env.evaluateJs(domSelector, /* istanbul ignore next */ function(domSelector) {
    ...
    async _injectBrowserRule(rule, offset, actionsResults) {
        ...
        await waitForEvaluate(this._env,
          () => {
