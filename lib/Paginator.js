...
Paginator.prototype = {
    ...
    _paginatePage: function () {
        ...
        return this._env
            ...
            .then(function () {
                var loadPagePromise = this._env.waitForPage(this._timeout);
                var ajaxPagePromise = this._hasPageDataChanged(previousPageHtml);
                var clickPromise = this._actions.click(selector);

                return vow.all([clickPromise,
                    vow.any([loadPagePromise, ajaxPagePromise])
                ]);
            }, this)
            .then(function () {
                debug('page %s opened', this._currentPage);
